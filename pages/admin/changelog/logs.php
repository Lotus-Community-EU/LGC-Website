                ...
                <?php
                    ...
                    while($log = $all_logs->fetch_array()) {
                        ...
                        <tr <?= ($user->hasPermission('admin_changelog_log_delete') && $log['deleted'] == 1) ? 'class="table-danger"' : '';?>>
                            ...
                            <td>
                                <a href="/user/<?= $log['user'];?>" <?= ($user->hasPermission('admin_changelog_log_delete') && $log['deleted'] == 1) ? 'class="text-dark"' : 'class="text-white"';?> target="_blank">Hyperlink (<?= $log['user'];?>)</a>
                                <?php if($log['deleted'] == 1) {?>
                                    | <a href="/user/<?= $log['deleted_by'];?>" <?= ($user->hasPermission('admin_changelog_log_delete') && $log['deleted'] == 1) ? 'class="text-dark"' : 'class="text-white"';?> target="_blank">Deleted by (<?= $log['deleted_by'];?>)</a>
                                <?php }?>
                            </td>
                            <td>
                                <?= $log['changed_what'];?>
